...
function Get_Platinum; function Equip_Menu; function Cm;
	...
	case 1:
		...
		if (Mission_Points < .MP_Jobchange) {
			mes "I am sorry but you don't look skilled enough.";
			mes "Talk to my friend ^990000Zeph^000000 and do a few more ^990000Hunting Quests^000000 before you can continue.";
			mes "You will need ^990000 " + (.MP_JobChange - Mission_Points) + "^000000 more.";
			close;
		}
		set .@eac, eaclass();
		...
		function Job_Menu {
			while(1) {
				if (Mission_Points < .MP_Jobchange) {
					mes "Pardon me? Are you trying yo cheat me? This incident is being reported to the staff.";
					logmes "ALL-In-ONE NPC : Somehow the player managed to get to the Job_Menu without Hunting Points enough.";
					close; }
				if (getargcount() > 1) {
					mes "Select a job.";
					set .@menu$,"";
				...
				if (select(" ~ Change into ^0055FF"+jobname(.@i)+"^000000 class: ~ ^777777"+((getargcount() > 1)?"Go back":"Cancel")+"^000000") == 1) {
					...
					set Mission_Points, Mission_Points - .MP_JobChange;
					...
					set Class_Mastery, Class_Mastery | pow(2,.@indice);
					debugmes "Classe escolhida: " + jobname(.@i) + " Posicao na array: " + .@indice + " e Class_Mastery recebeu: " + pow(2,.@indice);
					// ---
...
//	set .@MenuOption,1|2|4|8|16|32|64|128|256|512; biali
	set .@MenuOption,1|2|4|32|64|512;
	...
	set .SNovice,99;			// Minimum base level to turn into Super Novice
	...
	set .n$, "[Alberteen]";	// NPC Name
	
	//Biali Compatibilidade com o Class Mastery
...
rent_mb,153,74,6	duplicate(All-in-One NPC)	Ralph#mb	953
rent_ma,153,74,6	duplicate(All-in-One NPC)	Aleph#ma	953